...
(cl-defmethod forge--pull-topic ((repo forge-gitlab-repository) n)
  (let ((orig-buffer (current-buffer)))
    (if (forge-issue-p forge-buffer-topic)
        (forge--fetch-issue repo (forge-get-issue n) #'forge--update-issue)
...
(cl-defmethod forge--fetch-issue
  ((repo forge-gitlab-repository) issue callback)
  (let ((cb (let (cur done)
              (lambda (cb &optional v)
                (setq cur v)
                (if done
                    (funcall callback repo cur)
                  (cond
                   ;; fetch posts
                   ((not (assq 'posts cur))
                    (forge--fetch-issue-posts repo cur cb))
                   ...
                   (t (setq done t)
                      (funcall cb cb cur))))))))
    (forge--glab-get repo (format "/projects/:project/issues/%d"
                                  (oref issue number))
      `((per_page . 100))
      :unpaginate t
      :callback (lambda (value _headers _status _req)
...
(cl-defmethod forge--fetch-pullreq
  ((repo forge-gitlab-repository) pullreq callback)
  (let ((cb (let (cur done)
              (lambda (cb &optional v)
                (setq cur v)
                (if done
